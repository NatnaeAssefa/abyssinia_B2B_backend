...
import { ProductImageService } from "../../services/MarketPlace";
...
class ProductImageController {
  static findMany(request: Request, response: Response) {
    ...
    ProductImageService.findMany(parsedQuery.query, parsedQuery.paranoid)
      .then((result) => {
  ...
  static findOne(request: Request, response: Response) {
    ...
    ProductImageService.findOne(parsedQuery.query, parsedQuery.paranoid)
      .then((result) => {
        ServerResponse(request, response, 200, result, "", startTime);
      })
      .catch((error) => {
  ...
  static findById(request: Request, response: Response) {
    ...
    if (!error) {
      ...
      ProductImageService.findById(id, parsedQuery.query, parsedQuery.paranoid)
        .then((result) => {
          if (result) {
            ServerResponse(request, response, 200, result, "", startTime);
          } else {
  ...
  static create(request: any, response: Response) {
    ...
    if (!error) {
      ...
      ProductImageService.create(user, {
        ...data,
      })
        .then((result) => {
  ...
  static update(request: any, response: Response) {
    ...
    if (!error) {
      ...
      const data: any = request.body;
      const user: User = request.user;
      ProductImageService.update(user, id, data)
        .then((result) => {
  ...
  static delete(request: any, response: Response) {
    ...
    if (!error) {
      ...
      const user: User = request.user;
      ProductImageService.delete(user, id, null, force)
        .then((result) => {
          ServerResponse(request, response, 200, result, "Success", startTime);
        })
        .catch((error) => {
  ...
  static restore(request: any, response: Response) {
    ...
    if (!error) {
      const id: string = request.body.id;
      const user: User = request.user;
      ProductImageService.restore(user, id)
        .then((result) => {
          ServerResponse(request, response, 200, result, "Success", startTime);
        })
        .catch((error) => {
