...
import { ProductUseCaseService } from "../../services/MarketPlace";
...
class ProductUseCaseController {
  static findMany(request: any, response: Response) {
    const startTime = new Date();
    let parsedQuery: any = ParseQuery(request.query);

    ProductUseCaseService.findMany(parsedQuery.query, parsedQuery.paranoid)
      .then((result) => {
  ...
  static findSelectedProductUseCase(request: any, response: Response) {
    const startTime = new Date();
    let parsedQuery: any = ParseQuery(request.query);
    const excludeProductUseCases = ["Admin", "Super Admin"];
    parsedQuery.query.where = {
      ...parsedQuery.query.where,
      name: { [Op.notIn]: excludeProductUseCases },
    };
  ...
  static findOne(request: any, response: Response) {
    ...
    ProductUseCaseService.findOne(parsedQuery.query, parsedQuery.paranoid)
      .then((result) => {
        ServerResponse(request, response, 200, result, "", startTime);
      })
      .catch((error) => {
  ...
  static findById(request: any, response: Response) {
    ...
    if (!error) {
      ...
      ProductUseCaseService.findById(
        id,
        parsedQuery.query,
  ...
  static create(request: any, response: Response) {
    ...
    if (!error) {
      ...
      ProductUseCaseService.create(user, data)
        .then((result) => {
  ...
  static update(request: any, response: Response) {
    ...
    if (!error) {
      ...
      const data: any = request.body;
      const user: User = request.user;
      ProductUseCaseService.update(user, id, data)
        .then((result) => {
  ...
  static delete(request: any, response: Response) {
    ...
    if (!error) {
      ...
      const user: User = request.user;
      ProductUseCaseService.delete(user, id, null, force)
        .then((result) => {
          ServerResponse(request, response, 200, result, "Success", startTime);
        })
        .catch((error) => {
  ...
  static restore(request: any, response: Response) {
    ...
    if (!error) {
      const id: string = request.body.id;
      const user: User = request.user;
      ProductUseCaseService.restore(user, id)
        .then((result) => {
          ServerResponse(request, response, 200, result, "Success", startTime);
        })
        .catch((error) => {
...
export default ProductUseCaseController;
