...
import { ProductSpecificationService } from "../../services/MarketPlace";
...
class ProductSpecificationController {
  static findMany(request: any, response: Response) {
    ...
    ProductSpecificationService.findMany(parsedQuery.query, parsedQuery.paranoid)
      .then((result) => {
  ...
  static findSelectedProductSpecification(request: any, response: Response) {
    const startTime = new Date();
    let parsedQuery: any = ParseQuery(request.query);
    const excludeProductSpecifications = ["Admin", "Super Admin"];
    parsedQuery.query.where = {
      ...parsedQuery.query.where,
      name: { [Op.notIn]: excludeProductSpecifications },
    };
  ...
  static findOne(request: any, response: Response) {
    ...
    ProductSpecificationService.findOne(parsedQuery.query, parsedQuery.paranoid)
      .then((result) => {
        ServerResponse(request, response, 200, result, "", startTime);
      })
      .catch((error) => {
  ...
  static findById(request: any, response: Response) {
    ...
    if (!error) {
      ...
      ProductSpecificationService.findById(
        id,
        parsedQuery.query,
  ...
  static create(request: any, response: Response) {
    ...
    if (!error) {
      ...
      ProductSpecificationService.create(user, data)
        .then((result) => {
  ...
  static update(request: any, response: Response) {
    ...
    if (!error) {
      ...
      const data: any = request.body;
      const user: User = request.user;
      ProductSpecificationService.update(user, id, data)
        .then((result) => {
  ...
  static delete(request: any, response: Response) {
    ...
    if (!error) {
      ...
      const user: User = request.user;
      ProductSpecificationService.delete(user, id, null, force)
        .then((result) => {
          ServerResponse(request, response, 200, result, "Success", startTime);
        })
        .catch((error) => {
  ...
  static restore(request: any, response: Response) {
    ...
    if (!error) {
      const id: string = request.body.id;
      const user: User = request.user;
      ProductSpecificationService.restore(user, id)
        .then((result) => {
          ServerResponse(request, response, 200, result, "Success", startTime);
        })
        .catch((error) => {
