...
import { ActionLogService, UserService } from "../../services/User";
...
class UserController {
  ...
  static create(request: any, response: Response) {
    ...
    const schema = Joi.object({
      ...
      type: Joi.string()
        ...
        .required(),
      role_id: Joi.string().guid().allow(null),
      preferred_contact_method: Joi.string().trim().valid(...Object.values(PREFERRED_CONTACT_METHOD)),
    });
  ...
  static async getMe(request: any, response: Response) {
    const startTime = new Date();
    const user: User = request.user;
    const fullUserData = await UserDAL.findById(user.id, { include: [ {model: Cart}, {model: Role}] });
    ServerResponse(request, response, 200, fullUserData, "Success", startTime);
  }
  ...
  static update(request: any, response: Response) {
    ...
    const schema = Joi.object({
      ...
      status: Joi.string()
        ...
        .allow(null),
      role_id: Joi.string().guid().allow(null),
      notification_status: Joi.string()
        .valid(...Object.values(UserNotificationStatus))
        .allow(null),
      preferred_contact_method: Joi.string().valid(...Object.values(PREFERRED_CONTACT_METHOD)),
    });
  ...
  static toggleIsVerified(request: any, response: Response) {
    const startTime = new Date();
    const schema = Joi.object({
      id: Joi.string().guid().required(),
    ...
    if (!error) {
      ...
      const data: any = request.body;
      const user: User = request.user;
      UserService.toggleIsVerified(user, id, data)
        .then((result) => {
          ServerResponse(request, response, 200, result, "Success", startTime);
        })
        .catch((error) => {
